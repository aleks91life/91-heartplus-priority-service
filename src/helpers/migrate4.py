        ...
        if "prisma" in filters_dict and "searchState" in filters_dict["prisma"]:
            ...
            for condition in filters_dict["prisma"]["searchState"]:
                new_condition = {
                    ...
                    "operator": convert_operator(condition.get("operator", "eq")),
                    "value": [condition.get("value")] if condition.get("value") is not None else []
                }
                new_format["conditions"].append(new_condition)
    
    return new_format

def convert_operator(old_operator):
    operator_mapping = {
        " ": "eq",
        "=": "eq",
        "_not": "neq",
    ...
    return operator_mapping.get(old_operator, "eq")
...
def convert_to_new_rule(old_rule):
    ...
    if old_rule["patientSpecific"]:
        ...
        patient_rules["includedPatients"] = filters_dict["excludedPatients"]
