...
def convert_filters_to_new_format(filters, position, value_range):
    new_format = {
        "operator": "and",
        "conditions": []
    }
    ...
    # Handle position and value_range together
    if position and value_range:
        position_dict = json.loads(position)
        value_range_dict = json.loads(value_range)
        ...
        for key, value in position_dict.items():
            ...
            new_condition["conditions"].append(sub_condition)
        
        for key, value in value_range_dict.items():
            operator = "lte" if key.endswith("_max") else "gte"
            field_name = key.replace("_max", "").replace("_min", "")
            sub_condition = {
...
def convert_to_new_rule(old_rule):
    ...
    new_rule = {
        ...
        "interrogationRules": convert_filters_to_new_format(
            old_rule["filters"],
            old_rule["position"],
            old_rule["valueRange"]
        ),
